...
export default function RunDetail() {
	...
	if (!run) {
		return (
			<div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center">
				<div className="text-center">
					<h2 className="text-2xl font-bold text-slate-900 mb-4">Run not found</h2>
					<p className="text-slate-600 mb-6">The run you're looking for doesn't exist.</p>
					<button
						onClick={() => navigate('/')}
				...
				<div className="relative max-w-4xl mx-auto px-4 py-16">
					<div className="text-center">
						<h1 
							...
							style={{ color: run.detail.headerCard.titleColor }}
						>
						...
						<div className="flex flex-wrap justify-center gap-4 mt-8">
							{run.properties?.activityType && (
								<span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-white bg-opacity-20 text-neutral-800">
									📊 {run.properties.activityType}
								</span>
							)}
							{run.properties?.goal && (
								<span className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-white bg-opacity-20 text-neutral-800">
									🎯 {run.properties.goal} {run.properties.activityType === 'DURATION' ? 'seconds' : 'units'}
								</span>
							)}
						</div>
					</div>
									...
									<div className="px-8 py-6">
										...
										<div className="flex flex-col md:flex-row items-start gap-6">
											...
											<div className="flex-1">
												<p className="text-slate-600 mb-6">Listen to the perfect soundtrack for this run:</p>
